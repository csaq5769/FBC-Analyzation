 ...
 * This is important for i. e. checking proper state-parameter use
 *
 * @brief SqlHandler::followRedirectUri
 * @param str_website
 * @return strlst_resultingTraffic
 *         A QStringList holding the request to the redirect uri of the client or an empty QStringList if no such request was found
 */
QStringList SqlHandler::followRedirectUri(QString str_website)
{
    // pointer to db
    QSqlDatabase::database();
    ...
    QStringList strlst_trafficPermissionsAccepted = SqlHandler::onAcceptPermissionsInPermissionsDialogKlicked(str_website);
    QString str_idPermissionsAccepted;
    if(!strlst_trafficPermissionsAccepted.isEmpty())
    {
        str_idPermissionsAccepted = strlst_trafficPermissionsAccepted.at(0);
    }

    if(!str_redirectUri.isEmpty() && str_idPermissionsAccepted.toInt() != 0)
    {
        if(str_redirectUri.startsWith("https://staticxx.facebook.com") || str_redirectUri.startsWith("http://staticxx.facebook.com"))
        {
            QString str_requestContentPermissionsAccepted = strlst_trafficPermissionsAccepted.at(4);
            ...
            QSqlQuery queryStaticxxRedirect("SELECT * FROM "+str_tableName+" WHERE request_method LIKE '%"+str_domain+"%' AND id>"+str_idPermissionsAccepted);
        ...
        {
            // goto redirect uri
            QSqlQuery queryRedirectUri("SELECT * FROM "+str_tableName+" WHERE request_method LIKE '%"+str_redirectUri+"%' AND id>"+str_idPermissionsAccepted);
